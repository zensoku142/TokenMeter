...
- **本机统计与导出**：从顶部“切换视图”打开后自动读取 Codex（含归档）/ Claude 日志，先显示缓存，再后台更新；页面可见时每分钟增量刷新。支持 Top 排行、每日模型对比、图表/表格切换、自定义日期和项目筛选，并可后台导出 Excel（`.xlsx`），包含统计汇总、用量明细与口径说明。可显式选择已可访问的 WSL 目录，不启动 WSL。统计仅代表所选本机日志，账户归属未知，不作为订阅额度或实际账单。
