...
- **本机统计与导出**：从顶部“切换视图”打开本机统计，手动扫描 Codex（含归档）/ Claude 日志，按模型、项目、日期或会话查看 Token，并导出当前筛选的 JSON / CSV。支持显式选择目录，包括已可访问的 WSL 路径；不会启动 WSL。统计仅代表所选本机日志，账户归属未知，不作为订阅额度或实际账单。
