...
- **多平台总览**：从面板底部进入，同屏查看已配置平台的独立额度、余额与最后成功更新时间；手动刷新总览逐个采集并遵守失败退避，默认后台同步策略保持不变。失效连接可从对应卡片进入账户设置。
- **本机统计与导出**：从面板底部手动扫描 Codex（含归档）/ Claude 日志，按模型、项目、日期或会话查看 Token，并导出当前筛选的 JSON / CSV。支持显式选择目录，包括已可访问的 WSL 路径；不会启动 WSL。统计仅代表所选本机日志，账户归属未知，不作为订阅额度或实际账单。
