...
当前版本：**v1.15.2** · 桌宠扩展 **v0.1.4** · [产品官网（GitHub Pages）](https://zensoku142.github.io/TokenMeter/)

## 功能

- **多平台总览**：从顶部“切换视图”进入，同屏查看已配置平台的独立额度、余额与最后成功更新时间；打开后自动逐个采集，页面可见时每分钟更新，遵守失败退避并跳过刚刷新过的平台。离开后停止额外轮询，失效连接可从对应卡片进入账户设置。
- **独立账户档案**：在“设置 → 多账户”为同一平台保存多个连接，“账户连接”页也提供管理入口。支持添加、编辑、删除和独立监控；明确指定凭据或登录路径，秘密字段保存在 Windows 凭据管理器。档案不覆盖默认连接或 CLI 登录，迟到请求按档案版本和实际账号身份校验；额度通知可定位对应档案。
...
- 主程序 v1.15.2 配合桌宠 v0.1.4，可同步额度气泡与独立额度窗口的明暗底色、文字和低额度提示色；旧版仍可连接。
...
主程序 `1.15.2`，可选桌宠扩展 `0.1.4`。更新记录与校验文件见 [GitHub Releases](https://github.com/zensoku142/TokenMeter/releases)。
